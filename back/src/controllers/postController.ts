...
export const getPosts = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (postId) {
      ...
      const updatedViewCountPost = await postService.updatePostViewCount(postId);
...
export const deletePost = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const post = await postService.getPostByPostId(postId);
    if (!post) return res.status(404).json({ message: "존재하지 않는 게시글입니다." });
    if (post.authorId !== (req.user as User).id && !(req.user as User).manager)
      return res.status(403).json({ message: "권한이 없습니다." });
    const deletedPost = await postService.deletePostAndComments(postId);
    return res.status(200).json({ message: `${deletedPost}번 게시글이 삭제되었습니다.` });
  } catch (error) {
    console.error(error);
    return next(error);
  }
