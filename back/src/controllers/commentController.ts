...
export const createComment = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const newComment = await commentService.createComment(userId, postId, parentId, content);
...
export const deleteComment = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const deletedComments = await commentService.deleteCommentAndChildren(commentId);
    return res.status(200).json({ message: `${deletedComments}번 댓글이 삭제되었습니다.` });
  } catch (error) {
    console.error(error);
    return next(error);
  }
