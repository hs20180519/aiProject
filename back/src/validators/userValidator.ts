...
export const validateCheckEmail = (req: Request, res: Response, next: NextFunction) => {
  const schema = Joi.object({
    email: Joi.string().email().regex(emailPattern).optional(),
  });
  const { error } = schema.validate(req.query);
  if (error) return res.status(400).json({ validator: "이메일 중복 검사 전 유효성 검사 실패." });
  next();
};
...
export const validateRegister = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (error)
    return res.status(400).json({ validator: "이메일 인증 메일 발송 전 유효성 검사 실패." });
  next();
};
...
export const validateVerify = (req: Request, res: Response, next: NextFunction) => {
  ...
  const { error } = schema.validate(req.body);
  if (error) return res.status(400).json({ validator: "이메일 인증 전 유효성 검사 실패." });
  next();
};
...
export const validateCreateUser = (req: Request, res: Response, next: NextFunction) => {
  ...
  const { error } = schema.validate(req.body);
  if (error) return res.status(400).json({ validator: "회원가입 전 유효성 검사 실패." });
  next();
};
...
export const validateLogin = (req: Request, res: Response, next: NextFunction) => {
  ...
  const { error } = schema.validate(req.body);
  if (error) return res.status(400).json({ validator: "로그인 전 유효성 검사 실패." });
  next();
};
...
export const validateEditUser = (req: Request, res: Response, next: NextFunction) => {
  ...
  const { error } = schema.validate(req.body);
  if (error) return res.status(400).json({ validator: "회원정보 수정 전 유효성 검사 실패." });
  next();
};
